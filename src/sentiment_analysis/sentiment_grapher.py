#!/usr/bin/env python3
"""
Sentiment analysis visualization module.
...
def filter_data_by_time(df: pd.DataFrame, interval_minutes: str | int) -> pd.DataFrame:
...
def calculate_discrete_rolling_average(
    df: pd.DataFrame, window_minutes: int
) -> pd.DataFrame:
...
def create_sentiment_chart(
    ...
    if "step_times" in df.attrs and "step_values" in df.attrs:
        ...
        for boundary_time in step_times[
            1:-1
        ]:  # Skip first and last to avoid edge clutter
    ...
    if title:
        ...
        if start_dt.date() == end_dt.date():
            ...
            time_range = (
                f"{start_dt.strftime('%b %d, %H:%M')} - {end_dt.strftime('%H:%M')}"
            )
        else:
...
__all__ = [
    "calculate_discrete_rolling_average",
    "convert_timestamps",
    "create_charts",
    "create_sentiment_chart",
    "determine_chart_split",
